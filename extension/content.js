...
async function simulateHumanEngagement() {
...
function extractListings() {
  ...
  let elements = document.querySelectorAll('[data-obid]');
  ...
  elements.forEach((el, index) => {
    try {
      let exposeId = el.getAttribute('data-obid')
      ...
      if (exposeId && url) {
        const title = el.querySelector('h2, h3, h4, [class*="title"], [data-testid="headline"]')?.textContent?.trim() || 'Unknown';
...
function detectListingType() {
  ...
  const hasContactButton = hasContactForm || !!Array.from(document.querySelectorAll('button, a'))
    .find(btn => {
      if (btn.type === 'submit') return false;
      const text = btn.textContent.toLowerCase();
      return text.includes('nachricht schreiben') || text.includes('kontaktieren') || text.includes('anfrage senden') || text.includes('interesse bekunden');
    });
...
function extractLandlordName() {
  ...
  const isPrivate = !!document.querySelector('[class*="private-offer-logo"], [class*="PrivateOfferLogo"]')
...
function extractListingDetails() {
  ...
  for (const [field, cssKeys] of Object.entries(primaryFields)) {
    for (const cssKey of cssKeys) {
      ...
      if (el) {
        ...
        details[field] = img ? img.alt : el.textContent.trim();
  ...
  if (!details.internet) {
    ...
    if (container) {
      const speedText = Array.from(container.querySelectorAll('span'))
        .find(s => /\d+\s*MBit/i.test(s.textContent));
  ...
  document.querySelectorAll('dd[class*="is24qa-"]').forEach(dd => {
    ...
    if (qaClass) {
      ...
      if (!matchedKeys.has(key)) {
        ...
        extraAttrs[key] = img ? img.alt : dd.textContent.trim();
      }
    }
  });
  ...
  document.querySelectorAll('dl').forEach(dl => {
    ...
    dts.forEach(dt => {
      ...
      if (dd?.tagName === 'DD' && label && label.length < 60) {
        const img = dd.querySelector('img');
        const value = img ? img.alt : dd.textContent.trim();
...
async function detectCaptcha() {
  ...
  const captchaHeading = Array.from(document.querySelectorAll('h1, h2, h3, h4, p, span'))
    .find(el => el.textContent.includes('Sicherheitsabfrage'));
  ...
  const imgEl = captchaImg || document.querySelector('img[src*="captcha"], img[src*="getimage"]');
  if (!imgEl || !imgEl.src) {
  ...
  try {
    ...
    return { hasCaptcha: true, imageBase64: null, error: e.message };
  }
}

async function fillCaptchaAndSubmit(text) {
  if (!text) return { success: false, error: 'No captcha text provided' };

  try {
    // Find captcha input
    const captchaInput = document.querySelector('input#userAnswer, input[data-testid="userAnswer"], input[name="captcha"]');
    if (!captchaInput) {
    ...
    let submitBtn = document.querySelector('button[data-testid="captcha-submit"]');

    if (!submitBtn) {
      // Walk up from the captcha input to its containing form/modal and find a button there
      const captchaForm = captchaInput.closest('form') || captchaInput.closest('[role="dialog"], [class*="modal"], [class*="captcha"]');
      if (captchaForm) {
    ...
    console.log('[IS24] Clicking captcha submit:', submitBtn.textContent?.trim(), '| testid:', submitBtn.getAttribute('data-testid'));
    submitBtn.click();
    ...
    while (Date.now() - startTime < 8000) {
      ...
      const messageSent = document.querySelector('[class*="status-confirm"], [class*="StatusMessage_status-confirm"]') ||
        Array.from(document.querySelectorAll('div, span, h4'))
          .find(el => el.textContent.includes('Nachricht gesendet'));
      if (messageSent) {
      ...
      const stillVisible = document.querySelector('.captcha-image-container img, img[src*="captcha"]') ||
...
async function sendMessageToLandlord(message, formValues = {}, autoSend = true) {
  ...
  try {
        ...
        if (!contactBtn) {
          contactBtn = Array.from(document.querySelectorAll('button, a'))
            .find(btn => {
              if (btn.type === 'submit') return false;
              const text = btn.textContent.toLowerCase();
              return text.includes('nachricht schreiben') || text.includes('nachricht senden') ||
                text.includes('kontaktieren') || text.includes('anfrage senden') ||
        ...
        submitBtn = Array.from(document.querySelectorAll('button'))
          .find(btn => {
            ...
            return text.includes('abschicken') || text.includes('senden') ||
              text.includes('absenden') || text.includes('interesse bekunden') ||
              text.includes('bewerbung') || text.includes('anfrage');
          });
      }
    }
    ...
    while (Date.now() - startTime < 8000) {
      ...
      const captchaVisible = document.querySelector('.captcha-image-container img, img[src*="captcha"]') ||
        Array.from(document.querySelectorAll('h1, h2, h3, h4, p, span'))
          .find(el => el.textContent.includes('Sicherheitsabfrage'));
      if (captchaVisible) {
      ...
      if (formGone) {
        ...
        await sleep(500);
        const captchaAfter = document.querySelector('.captcha-image-container img, img[src*="captcha"]') ||
          Array.from(document.querySelectorAll('h1, h2, h3, h4, p, span'))
            .find(el => el.textContent.includes('Sicherheitsabfrage'));
        if (captchaAfter) {
    ...
    const captchaFinal = document.querySelector('.captcha-image-container img, img[src*="captcha"]') ||
      Array.from(document.querySelectorAll('h1, h2, h3, h4, p, span'))
        .find(el => el.textContent.includes('Sicherheitsabfrage'));
    if (captchaFinal) {
...
function extractPaginationInfo() {
  ...
  if (pageLinks.length > 0) {
    ...
    pageLinks.forEach(link => {
      const match = link.href?.match(/pagenumber=(\d+)/);
...
async function fillConversationReply(message) {
  ...
  if (isContentEditable) {
    ...
    for (let i = 0; i < message.length; i += chunkSize) {
      input.value += message.slice(i, i + chunkSize);
      input.dispatchEvent(new Event('input', { bubbles: true }));
  ...
  switch (request.action) {
    ...
    case 'detectCaptcha':
      detectCaptcha()
        ...
        .catch(error => sendResponse({ hasCaptcha: false, error: error.message }));
      return true;
    ...
    case 'checkMessageSent':
      ...
      {
        const successEl = document.querySelector('[class*="status-confirm"], [class*="StatusMessage_status-confirm"]') ||
          Array.from(document.querySelectorAll('div, span, h1, h2, h3, h4, p'))
            .find(el => el.textContent.includes('Nachricht gesendet') || el.textContent.includes('Nachricht wurde gesendet'));
        const contactForm = document.querySelector('textarea[name="message"], form[data-testid="contact-form"]');
        const captchaEl = document.querySelector('.captcha-image-container img, img[src*="captcha"]') ||
          Array.from(document.querySelectorAll('h1, h2, h3, h4, p, span'))
            .find(el => el.textContent.includes('Sicherheitsabfrage'));
        sendResponse({
